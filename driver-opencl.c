        ...
        int64_t __maybe_unused max_nonce)
{
  ...
  if (unlikely(status != CL_SUCCESS)) {
    applog(LOG_ERR, "Error %d: Enqueueing MAIN kernel onto command queue. (clEnqueueNDRangeKernel)", status);
    return -1;
  }
  ...
  for (i = 0; i < clState->n_extra_kernels; i++) {
      status = clEnqueueNDRangeKernel(clState->commandQueue, clState->extra_kernels[i], 1, p_global_work_offset,
                      globalThreads, localThreads, 0,  NULL, NULL);
      if (unlikely(status != CL_SUCCESS)) {
          applog(LOG_ERR, "Error %d: Enqueueing EXTRA kernel %d onto command queue. (clEnqueueNDRangeKernel)", status, i);
          return -1;
      }
  }
